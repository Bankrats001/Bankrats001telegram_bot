...
def binchk(message):
    ...
    if len(args) < 2:
        return bot.reply_to(message, "❌ Usage: /binchk <bin>")
...
def fakegen(message):
    names = ["Liam Johnson", "Emily Smith", "Noah Brown", "Ava Wilson"]
    streets = ["221B Baker St", "742 Evergreen Terrace", "1600 Pennsylvania Ave"]
    cities = ["New York", "Miami", "Dallas", "Los Angeles"]
    states = ["NY", "FL", "TX", "CA"]
...
@bot.message_handler(commands=['manualpay'])
def manualpay(message):
    msg = f"<b>💸 MANUAL PAYMENT</b>\n\n"
...
def feedback(message):
    bot.send_message(message.chat.id, "💬 Drop your feedback message and we’ll review.")
    bot.register_next_step_handler(message, save_feedback)

def save_feedback(msg):
    bot.send_message(ADMIN_ID, f"📩 FEEDBACK from @{msg.from_user.username or 'NoUser'} ({msg.from_user.id}):\n\n{msg.text}")
    bot.send_message(msg.chat.id, "✅ Thanks! Your feedback has been submitted.")

# ========== /DISCLAIMER ==========

@bot.message_handler(commands=['disclaimer'])
def disclaimer(message):
    msg = f"<b>⚠️ DISCLAIMER</b>\n\n"
    ...
    msg += "By using this bot, you agree to this disclaimer."
    bot.send_message(message.chat.id, msg, parse_mode='HTML')
